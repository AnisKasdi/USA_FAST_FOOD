...
const emptyItem = (): Omit<MenuItem, "id"> => ({
  ...
  price: 0,
  category: "Specials",
  image: "/images/CharcoalChicken_Hero.jpg",
  calories: 0,
  allergens: [],
  isNew: false,
});
...
export default function MenuTab({ menu, setMenu }: { menu: MenuItem[]; setMenu: (m: MenuItem[]) => void }) {
  ...
  const saveEdit = () => {
    if (!editing) return;
    setMenu(menu.map((m) => (m.id === editing.id ? editing : m)));
    setEditing(null);
  };

  const saveNew = () => {
    const newItem: MenuItem = { ...draft, id: `custom-${Date.now()}` };
  ...
  const deleteItem = (id: string) => {
    if (confirm("Delete this item?")) setMenu(menu.filter((m) => m.id !== id));
  };

  const toggleNew = (id: string) => {
    setMenu(menu.map((m) => (m.id === id ? { ...m, isNew: !m.isNew } : m)));
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-6" style={{ background: "rgba(0,0,0,0.85)" }}>
      <div className="rounded-3xl p-8 w-full max-w-2xl overflow-y-auto max-h-[90vh]" style={{ background: "#1a1a1a", border: "1px solid #2a2a2a" }}>
        ...
        <div className="flex flex-col gap-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <Field label="Category">
              <select value={data.category ?? "Specials"} onChange={(e) => onChange("category", e.target.value)}
                className="field-input" style={fieldStyle}>
          ...
          <div className="grid grid-cols-3 gap-4">
            <Field label="Price ($)">
              <input type="number" step="0.01" value={data.price ?? 0} onChange={(e) => onChange("price", parseFloat(e.target.value))}
                style={fieldStyle} />
            </Field>
            <Field label="Calories">
              <input type="number" value={data.calories ?? 0} onChange={(e) => onChange("calories", parseInt(e.target.value))}
                style={fieldStyle} />
            </Field>
            <Field label="Badge NEW">
              <div className="flex items-center h-full pt-1">
                <input type="checkbox" checked={!!data.isNew} onChange={(e) => onChange("isNew", e.target.checked)}
          ...
          {data.image && (
            <img src={data.image} alt="preview" className="w-full h-40 object-cover rounded-xl" />
          )}
              ...
              {allergenOptions.map(a => {
                ...
                return (
                  <button
                    ...
                    type="button"
                    onClick={() => {
                      const cur = data.allergens ?? [];
                      onChange("allergens", selected ? cur.filter(x => x !== a) : [...cur, a]);
                    }}
        ...
        <div className="flex gap-3 mt-8">
          <button onClick={onSave} className="flex-1 py-3 rounded-xl font-bold" style={{ background: "#E63946", color: "#fff" }}>
            Save
          </button>
    ...
    <div className="flex flex-col gap-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm" style={{ color: "#555" }}>{menu.length} items total</p>
        </div>
        <button
                ...
                <td className="px-4 py-3">
                  <div className="flex gap-2">
                    ...
                    <button onClick={() => deleteItem(item.id)}
                      className="text-xs px-3 py-1.5 rounded-lg font-bold"
                      style={{ background: "rgba(230,57,70,0.15)", color: "#E63946" }}>Del</button>
                  </div>
                </td>
              </tr>
            ))}
      ...
      {adding && (
        <ItemForm
          ...
          onSave={saveNew}
          onCancel={() => { setAdding(false); setDraft(emptyItem()); }}
        />
      )}
    </div>
  );
