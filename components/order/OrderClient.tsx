...
export default function OrderClient() {
  ...
  const placeOrder = async () => {
    ...
    try {
      ...
      // order still progresses in UI even if DB write fails
    }
  ...
  if (status !== "idle") {
    return (
      <div className="max-w-2xl mx-auto text-center py-20">
          ...
          {[
            ...
            return (
              <div key={step.key} className="flex items-center gap-2">
                ...
                {i < arr.length - 1 && (
                  <div className="w-12 h-0.5 mb-5 transition-all duration-500" style={{ background: stepIdx < currentIdx ? "#E63946" : "#2a2a2a" }} />
                )}
              </div>
            );
          })}
        ...
        {status === "ready" && (
          <button
            className="mt-10 px-8 py-3 rounded-full font-bold"
            style={{ background: "#E63946", color: "#fff" }}
            onClick={() => { setCart([]); setStatus("idle"); }}
          >
            New Order
          </button>
        )}
      ...
      <div className="lg:col-span-2 flex flex-col gap-8">
        ...
        {mode === "delivery" && (
          <div className="rounded-2xl p-5" style={{ background: "#222", border: "1px solid #2a2a2a" }}>
            ...
            <p className="text-sm" style={{ color: "#555" }}>Powered by DoorDash & Uber Eats</p>
            <div className="flex gap-3 mt-3">
              <a
                href="#"
                className="px-4 py-2 rounded-xl text-sm font-bold"
                style={{ background: "#FF3008", color: "#fff" }}
              >
                DoorDash
              </a>
              <a
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  {item.isNew && (
                    <span className="text-xs font-bold px-2 py-0.5 rounded-full" style={{ background: "#FFB703", color: "#1A1A1A" }}>NEW</span>
                  )}
                </div>
      ...
      <div className="lg:col-span-1">
        ...
        >
              ...
              {cart.map((c) => (
                <div key={c.id} className="flex items-center justify-between gap-3">
                  ...
                  <button
                    ...
                    style={{ background: "#2d2d2d", color: "#E63946" }}
                  >
                    Remove
                  </button>
                </div>
          ...
          <div className="border-t pt-4 mt-2 flex flex-col gap-3" style={{ borderColor: "#2a2a2a" }}>
            ...
            <div className="flex justify-between text-sm" style={{ color: "#888" }}>
              <span>Tax (9.5%)</span>
              <span>${(total * 0.095).toFixed(2)}</span>
            </div>
