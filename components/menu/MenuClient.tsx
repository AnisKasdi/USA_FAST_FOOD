...
const allergenColors: Record<string, string> = {
  Gluten: "#b45309", Dairy: "#1d4ed8", Eggs: "#ca8a04", Nuts: "#166534",
  Sesame: "#6b21a8", Fish: "#0e7490", Soy: "#15803d", Shellfish: "#9f1239",
};
    ...
    <>
      <div className="flex items-center gap-3 mb-8">
        <span className="inline-flex items-center gap-2 px-4 py-2 rounded-full text-sm font-bold"
          ...
          <ShieldCheck size={15} strokeWidth={2.5} /> 100% Halal Certified
        </span>
      ...
      <div className="flex flex-wrap gap-3 mb-10">
        {categories.map(cat => (
          <button key={cat} onClick={() => setActive(cat)} className="px-6 py-2.5 rounded-full text-sm font-bold transition-all duration-200"
            style={{ background: active === cat ? "#E63946" : "#222", color: active === cat ? "#fff" : "#888", border: active === cat ? "none" : "1px solid #333" }}>
            {cat}
          </button>
        ))}
      </div>
      ...
      {loading ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {Array.from({ length: 8 }).map((_, i) => (
            <div key={i} className="rounded-2xl h-72 animate-pulse" style={{ background: "#222" }} />
          ))}
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {filtered.map(item => (
            <div key={item.id}
              ...
              <div className="relative h-48 overflow-hidden">
                ...
                {item.isNew && (
                  <span className="absolute top-3 left-3 text-xs font-bold px-3 py-1 rounded-full" style={{ background: "#FFB703", color: "#1A1A1A" }}>NEW</span>
                )}
                <span className="absolute top-3 right-3 inline-flex items-center gap-1 text-xs font-bold px-2 py-1 rounded-full"
      ...
      {zoom && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-6" style={{ background: "rgba(0,0,0,0.85)" }}
          ...
          <div className="rounded-3xl overflow-hidden max-w-lg w-full relative" style={{ background: "#222" }}
            onClick={e => e.stopPropagation()}>
            <img src={zoom.image} alt={zoom.name} className="w-full h-72 object-cover" />
            <button className="absolute top-4 right-4 w-9 h-9 rounded-full flex items-center justify-center font-bold"
              style={{ background: "rgba(0,0,0,0.7)", color: "#fff" }} onClick={() => setZoom(null)}>×</button>
            <div className="p-6">
              <div className="flex items-center gap-3 mb-2">
                <h2 className="font-bebas text-3xl" style={{ color: "#F5F5F5" }}>{zoom.name}</h2>
                {zoom.isNew && <span className="text-xs font-bold px-3 py-1 rounded-full" style={{ background: "#FFB703", color: "#1A1A1A" }}>NEW</span>}
              </div>
              ...
              {zoom.allergens.length > 0 && (
                <div className="mb-6">
                  ...
                  <div className="flex gap-2 flex-wrap">{zoom.allergens.map(a => <AllergenBadge key={a} name={a} />)}</div>
                </div>
              )}
              <div className="flex items-center justify-between">
                <span className="font-bebas text-3xl" style={{ color: "#FFB703" }}>${zoom.price.toFixed(2)}</span>
                <a href="/commander" className="px-6 py-3 rounded-full font-bold transition-all hover:opacity-90"
                  style={{ background: "#E63946", color: "#fff" }}>Add to Order</a>
              </div>
            </div>
          </div>
        </div>
